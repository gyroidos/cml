...
panic_read_kernel_log(void)
{
	...
	if (copied_kernel_log_fd < 0) {
		ERROR_ERRNO("Could not open output file to copy kernel log to %s", copied_kernel_log);
	...
	while (bytes_read > 0) {
		...
		if (bytes_read == 0) {
			...
			if (bytes_written < 0) {
				ERROR_ERRNO("Could not copy kernel log to output file %s.", copied_kernel_log);
				break;
			}
		}
	}
		...
		while ((entry = readdir(directory)) != NULL) {
			...
			if (stat_buf.st_mtime) {
				...
				if (diff > MAX_DAYS_IN_SECONDS) {
					...
					if (ret) {
						ERROR_ERRNO("%s could not be removed", entry_name_with_path);
					}
				}
			}
			mem_free(entry_name_with_path);
			...
			if (strstr(entry->d_name, "kmsg") != NULL) {
				...
				if (stat_buf.st_mtime) {
					...
					if (diff < smallest_diff) {
						smallest_diff = diff;
						filename_of_latest_kmsg = mem_strdup(entry_name_with_path);
					}
				}
				mem_free(entry_name_with_path);
			}
