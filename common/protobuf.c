...
protobuf_send_message(int fd, const ProtobufCMessage* message)
{
	...
	if (-1 == bytes_sent)
		goto error_write;
	TRACE("sent protobuf message length (%d bytes sent, %zu bytes expected, len=%d)",
	...
	TRACE("sent protobuf message data (%d bytes sent, %d bytes expected)", bytes_sent, buflen);
...
protobuf_recv_message(int fd, const ProtobufCMessageDescriptor *descriptor)
{
	...
	if (0 == bytes_read) {// EOF / remote end closed the connection
		DEBUG("EOF reached on fd %d (remote end closed connection?)", fd);
		return NULL;
	}
	buflen = ntohl(buflen);
	TRACE("read protobuf message length (%d bytes read, %zu bytes expected, len=%d)",
	...
	TRACE("read protobuf message data (%d bytes read, %d bytes expected)", bytes_read, buflen);

	if ((size_t)bytes_read != buflen) {
		ERROR("Dropped protobuf message (expected length : %d bytes read != %d bytes expected)",
			bytes_read, buflen);
		mem_free(buf);
		goto error_read;
	}
