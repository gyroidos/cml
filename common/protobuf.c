...
ProtobufCMessage *
protobuf_unpack_message(const ProtobufCMessageDescriptor *descriptor, uint8_t *buf, uint32_t buflen)
{
	ASSERT(descriptor);
				   ...
				   const ProtobufCMessageDescriptor *descriptor, bool purge)
{
	...
	long fp = ftell(file);
	TRACE("Current message ends at at %ld", fp);
	...
	//if requested, remove read message from file
	if (purge) {
		char *buf = file_read_new(filename, 10000);

		//TODO implement w.r.t. available RAM in common/file.c
		if (!file) {
			ERROR_ERRNO("Failed to remove message from file");
			goto out;
		}

		if (file_write(filename, buf + fp, strlen(buf + fp))) {
			ERROR_ERRNO("Failed to remove message from file");
		}
	}
