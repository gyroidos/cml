...
uevent_parse_nl(uevent_event_t *uevent)
{
	...
	if (uevent_event_is_udev(uevent)) {
		...
		/*
		 * properties_off comes from untrusted user input; compute the
		 * bound without the 32-bit unsigned overflow that
		 * "properties_off + 32" would suffer (a near-UINT_MAX offset
		 * would wrap and pass).
		 */
...
uevent_replace_member(const uevent_event_t *uevent, const char *oldmember, const char *newmember)
{
	...
	/*
	 * Bail out if the replacement would overflow the fixed-size raw
	 * buffer; this must be checked before the copies below write into it.
	 * We need room for msg_len content bytes plus the terminating null
	 * char, i.e. msg_len must be strictly less than UEVENT_BUF_LEN.
	 */
...
uevent_handle(UNUSED int fd, UNUSED unsigned events, UNUSED event_io_t *io, UNUSED void *data)
{
	 ...
	 * Use a signed temporary: msg_len is size_t, so the "<= 0" error
	 * check must not be done on the assignment result, where -1 would
	 * become SIZE_MAX.
	 */
