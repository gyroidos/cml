...
network_move_link_ns(pid_t src_pid, pid_t dest_pid, const char *interface)
{
	...
	const char *const argv[] = { NSENTER_PATH, "-t",      src_pid_str, "-n",	 IP_PATH, "link",
				     "set",	interface, "netns",     dest_pid_str, NULL };
	DEBUG("NSENTER:");
...
network_setup_default_route_table(const char *table_id, const char *gateway, bool add)
{
	...
	const char *const argv[] = { "ip",     "route", add ? "replace" : "del", "default", "via", gateway, "table",
				     table_id, NULL };
	return network_fork_and_execvp(IP_PATH, argv);
}
...
network_setup_route_table(const char *table_id, const char *net_dst, const char *dev, bool add)
{
	...
	const char *const argv[] = { "ip",     "route", add ? "replace" : "del", net_dst, "dev", dev, "table",
				     table_id, NULL };
	return network_fork_and_execvp(IP_PATH, argv);
}
...
network_setup_route(const char *net_dst, const char *dev, bool add)
{
	...
	const char *const argv[] = { "ip", "route", add ? "replace" : "del", net_dst, "dev",
...
network_setup_masquerading(const char *subnet, bool enable)
{
	...
	const char *const argv[] = {
		IPTABLES_PATH, "-t",    "filter",  enable ? "-I" : "-D",  "FORWARD", "-d",     subnet,
...
network_route_localnet(const char *interface, bool enable)
{
	...
	if (file_write(route_localnet_file, enable ? "1" : "0", 1) <= 0) {
		ERROR_ERRNO("Could not %s localnet routing for %s", enable ? "enable" : "disable", interface);
		error = -1;
	}
