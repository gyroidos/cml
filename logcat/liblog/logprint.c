			  ...
			  const AndroidLogEntry *entry, size_t *p_outLength)
{
	...
	nsInfoLen = 0;
	if (entry->ns_initpid) {
		nsInfoLen = snprintf(nsInfoBuf, sizeof(nsInfoBuf), "|%*s(%5d)| ", DEV_NS_TAG_LEN,
	...
	if (p_format->colored_output == OUTPUT_COLOR_ON) {
		prefixColorLen = snprintf(prefixBufTmp, prefixBufTmpRemainLen, "%c[%d;%d;%dm", 0x1B, 38, 5,
	...
	case FORMAT_TAG:
		prefixLen = snprintf(prefixBufTmp, prefixBufTmpRemainLen, "%c/%-8s: ", priChar, entry->tag);
		strcpy(suffixBuf, "\n");
		suffixLen = 1;
		break;
	case FORMAT_PROCESS:
		prefixLen = snprintf(prefixBufTmp, prefixBufTmpRemainLen, "%c(%5d) ", priChar, entry->pid);
		suffixLen = snprintf(suffixBuf, sizeof(suffixBuf), "  (%s)\n", entry->tag);
		break;
	case FORMAT_THREAD:
		prefixLen =
			snprintf(prefixBufTmp, prefixBufTmpRemainLen, "%c(%5d:%5d) ", priChar, entry->pid, entry->tid);
		strcpy(suffixBuf, "\n");
		suffixLen = 1;
		break;
	case FORMAT_RAW:
		...
		break;
	case FORMAT_TIME:
		prefixLen = snprintf(prefixBufTmp, prefixBufTmpRemainLen, "%s.%03ld %c/%-8s(%5d): ", timeBuf,
				     entry->tv_nsec / 1000000, priChar, entry->tag, entry->pid);
		strcpy(suffixBuf, "\n");
		suffixLen = 1;
		break;
	case FORMAT_THREADTIME:
		prefixLen = snprintf(prefixBufTmp, prefixBufTmpRemainLen, "%s.%03ld %5d %5d %c %-8s: ", timeBuf,
				     entry->tv_nsec / 1000000, entry->pid, entry->tid, priChar, entry->tag);
		strcpy(suffixBuf, "\n");
		suffixLen = 1;
		break;
	case FORMAT_LONG:
		prefixLen = snprintf(prefixBufTmp, prefixBufTmpRemainLen, "[ %s.%03ld %5d:%5d %c/%-8s ]\n", timeBuf,
				     entry->tv_nsec / 1000000, entry->pid, entry->tid, priChar, entry->tag);
		strcpy(suffixBuf, "\n\n");
		suffixLen = 2;
	...
	case FORMAT_BRIEF:
	default:
		prefixLen = snprintf(prefixBufTmp, prefixBufTmpRemainLen, "%c/%-8s(%5d): ", priChar, entry->tag,
				     entry->pid);
		strcpy(suffixBuf, "\n");
		suffixLen = 1;
		break;
	...
	// extraneously
	bufferSize = (numLines * (nsInfoLen + prefixColorLen + prefixLen + suffixLen + suffixColorLen)) +
		     entry->messageLen + 1;
