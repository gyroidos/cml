...
__android_log_processLogBuffer(struct logger_entry_ns *buf, char *buf_msg, AndroidLogEntry *entry)
{
	...
	entry->pid = buf->pid;
	entry->tid = buf->tid;
	entry->ns_initpid = buf->ns_initpid;
	entry->ns_pid = buf->ns_pid;
	entry->ns_tid = buf->ns_tid;
	memcpy(entry->ns_tag, buf->ns_tag, DEV_NS_TAG_LEN);
	...
	entry->priority = buf_msg[0];
	entry->tag = buf_msg + 1;
	entry->message = buf_msg + msgStart;
 ...
 */
static int
android_log_printBinaryEvent(const unsigned char **pEventData, size_t *pEventDataLen,
			     char **pOutBuf, size_t *pOutBufLen)
{
 ...
 */
static int
__android_log_processBinaryLogBuffer_ns(struct logger_entry_ns *buf, const char *buf_msg,
					AndroidLogEntry *entry, const EventTagMap *map,
					char *messageBuf, int messageBufLen)
{
     ...
     * Pull the tag out.
     */
	eventData = (const unsigned char *)buf_msg;
	inCount = buf->len;
	if (inCount < 4)
		return -1;
	...
	int result;
	result = android_log_printBinaryEvent(&eventData, &inCount, &outBuf, &outRemaining);
	if (result < 0) {
...
int
android_log_processBinaryLogBuffer(struct logger_entry *buf, AndroidLogEntry *entry,
				   const EventTagMap *map, char *messageBuf, int messageBufLen)
{
	...
	return __android_log_processBinaryLogBuffer_ns(&ns_buf, buf->msg, entry, map, messageBuf,
						       messageBufLen);
}

int
android_log_processBinaryLogBuffer_ns(struct logger_entry_ns *buf, AndroidLogEntry *entry,
				      const EventTagMap *map, char *messageBuf, int messageBufLen)
{
	return __android_log_processBinaryLogBuffer_ns(buf, buf->msg, entry, map, messageBuf,
						       messageBufLen);
}
			  ...
			  const AndroidLogEntry *entry, size_t *p_outLength)
{
	...
	if (entry->ns_initpid) {
		nsInfoLen =
			snprintf(nsInfoBuf, sizeof(nsInfoBuf), "|%*s(%5d)| ", DEV_NS_TAG_LEN,
				 !entry->ns_tag[0] ? "<init>" : entry->ns_tag, entry->ns_initpid);
	}
	...
	if (p_format->colored_output == OUTPUT_COLOR_ON) {
		prefixColorLen = snprintf(prefixBufTmp, prefixBufTmpRemainLen, "%c[%d;%d;%dm", 0x1B,
					  38, 5, colorFromPri(entry->priority));
	...
	case FORMAT_TAG:
		prefixLen = snprintf(prefixBufTmp, prefixBufTmpRemainLen, "%c/%-8s: ", priChar,
				     entry->tag);
		strcpy(suffixBuf, "\n");
		suffixLen = 1;
		break;
	case FORMAT_PROCESS:
		prefixLen = snprintf(prefixBufTmp, prefixBufTmpRemainLen, "%c(%5d) ", priChar,
				     entry->pid);
		suffixLen = snprintf(suffixBuf, sizeof(suffixBuf), "  (%s)\n", entry->tag);
		break;
	case FORMAT_THREAD:
		prefixLen = snprintf(prefixBufTmp, prefixBufTmpRemainLen, "%c(%5d:%5d) ", priChar,
				     entry->pid, entry->tid);
		strcpy(suffixBuf, "\n");
		suffixLen = 1;
		break;
	...
	case FORMAT_THREADTIME:
		prefixLen =
			snprintf(prefixBufTmp, prefixBufTmpRemainLen,
	...
	case FORMAT_LONG:
		prefixLen = snprintf(prefixBufTmp, prefixBufTmpRemainLen,
				     "[ %s.%03ld %5d:%5d %c/%-8s ]\n", timeBuf,
				     entry->tv_nsec / 1000000, entry->pid, entry->tid, priChar,
				     entry->tag);
		strcpy(suffixBuf, "\n\n");
		suffixLen = 2;
	...
	case FORMAT_BRIEF:
	default:
		prefixLen = snprintf(prefixBufTmp, prefixBufTmpRemainLen, "%c/%-8s(%5d): ", priChar,
				     entry->tag, entry->pid);
		strcpy(suffixBuf, "\n");
		suffixLen = 1;
		break;
	...
	// extraneously
	bufferSize =
		(numLines * (nsInfoLen + prefixColorLen + prefixLen + suffixLen + suffixColorLen)) +
		entry->messageLen + 1;
...
android_log_printLogLine(AndroidLogFormat *p_format, int fd, const AndroidLogEntry *entry)
{
	...
	outBuffer = android_log_formatLogLine(p_format, defaultBuffer, sizeof(defaultBuffer), entry,
					      &totalLen);
