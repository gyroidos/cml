...
struct queued_entry_t {
    union {
        ...
        struct logger_entry_ns entry_ns __attribute__((aligned(4)));
    };
    queued_entry_t* next;
...
void printBinary(struct queued_entry_t *qbuf)
{
    size_t size = qbuf->elen + qbuf->lstruct_len;
    int ret;

    do {
        ret = write(g_outFD, qbuf->buf, size);
    } while (ret < 0 && errno == EINTR);
}

static void processBuffer(log_device_t* dev, struct queued_entry_t *qbuf)
{
    int bytesWritten = 0;
    int err;
    ...
    if (dev->binary) {
        if (dev->dev_ns_entry)
            err = android_log_processBinaryLogBuffer_ns(&qbuf->entry_ns, &entry, g_eventTagMap,
                                                        binaryMsgBuf, sizeof(binaryMsgBuf));
        else
            err = android_log_processBinaryLogBuffer(&qbuf->entry, &entry, g_eventTagMap,
                                                     binaryMsgBuf, sizeof(binaryMsgBuf));
        //printf(">>> pri=%d len=%d msg='%s'\n",
        //    entry.priority, entry.messageLen, entry.message);
    } else {
        if (dev->dev_ns_entry)
            err = android_log_processLogBuffer_ns(&qbuf->entry_ns, &entry);
        else
            err = android_log_processLogBuffer(&qbuf->entry, &entry);
    }
    if (err < 0) {
        goto error;
    }
...
static void printNextEntry(log_device_t* dev) {
    maybePrintStart(dev);
    if (g_printBinary) {
        printBinary(dev->queue);
    } else {
        processBuffer(dev, dev->queue);
    }
    skipNextEntry(dev);
}
            ...
            for (dev=devices; dev; dev = dev->next) {
                if (FD_ISSET(dev->fd, &readset)) {
                    ...
                    if (ret < 0) {
                        ...
                        if (dev->dev_ns_entry) {
                            entry->elen = entry->entry_ns.len;
                            entry->lstruct_len = sizeof(struct logger_entry_ns);
                            entry->msg = &entry->entry_ns.msg[0];
                        } else {
                            entry->elen = entry->entry.len;
                            entry->lstruct_len = sizeof(struct logger_entry);
                            entry->msg = &entry->entry.msg[0];
                        }
                    }

                    if (entry->elen != ret - entry->lstruct_len) {
                        fprintf(stderr, "read: unexpected length. Expected %d, got %d\n",
                                entry->elen, ret - entry->lstruct_len);
                        exit(EXIT_FAILURE);
                    }

                    entry->msg[entry->elen] = '\0';
...
static void show_help(const char *cmd)
{
    ...
    fprintf(stderr, "options include:\n"
                    ...
                    "  -C              colored output\n"
                    "  -A              Output interleaved logs from All running cells");
...
int main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        ret = getopt(argc, argv, "cdt:gsQf:r::n:v:b:ABC");
...
#define  CONSOLE_OPTION "androidboot.console="
    ...
    dev = devices;
    while (dev) {
        dev->fd = open(dev->device, mode | extra_oflags);
        if (dev->fd < 0) {
            fprintf(stderr, "Unable to open log device '%s': %s\n",
                dev->device, strerror(errno));
            exit(EXIT_FAILURE);
        }
        dev->dev_ns_entry = !!(extra_oflags & O_DIRECT);
