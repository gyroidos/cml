...
tpm2d_marshal_structure_new(void *structure, MarshalFunction_t marshal_function, size_t *size)
{
	...
	if (TPM_RC_SUCCESS !=
	    TSS_Structure_Marshal(&bin_stream, &written_size, structure, marshal_function)) {
...
tpm2_powerup(void)
{
	...
	if (TPM_RC_SUCCESS !=
	    (rc = TSS_TransmitPlatform(tss_context, TPM_SIGNAL_POWER_OFF, "TPM2_PowerOffPlatform")))
		goto err;

	if (TPM_RC_SUCCESS !=
	    (rc = TSS_TransmitPlatform(tss_context, TPM_SIGNAL_POWER_ON, "TPM2_PowerOnPlatform")))
		goto err;
...
tpm2_startup(TPM_SU startup_type)
{
	...
	rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_Startup,
			 TPM_RH_NULL, NULL, 0);
...
tpm2_selftest(void)
{
	...
	rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_SelfTest,
			 TPM_RH_NULL, NULL, 0);
...
tpm2_clear(const char *lockout_pwd)
{
	...
	rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_Clear,
...
tpm2_dictionaryattacklockreset(const char *lockout_pwd)
{
	...
	rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL,
			 TPM_CC_DictionaryAttackLockReset, TPM_RS_PW, lockout_pwd, 0, TPM_RH_NULL,
			 NULL, 0);
...
tpm2_policyauthvalue(TPMI_SH_POLICY se_handle)
{
	...
	rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_PolicyAuthValue,
			 TPM_RH_NULL, NULL, 0);
...
tpm2_policyrestart(TPMI_SH_POLICY se_handle)
{
	...
	rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_PolicyRestart,
			 TPM_RH_NULL, NULL, 0);
...
tpm2_flushcontext(TPMI_DH_CONTEXT handle)
{
	...
	rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_FlushContext,
			 TPM_RH_NULL, NULL, 0);
			...
			const char *file_name_pub_key, uint32_t *out_handle)
{
	...
	else if (TPM_RC_SUCCESS != (rc = TSS_TPM2B_StringCopy(&in.inSensitive.sensitive.userAuth.b,
	...
	if (TPM_RC_SUCCESS !=
	    (rc = tpm2_public_area_helper(&in.inPublic.publicArea, object_attrs, key_type)))
		return rc;
		 ...
		 const char *file_name_pub_key, const char *file_name_tss_key)
{
	...
	rc = TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out, (COMMAND_PARAMETERS *)&in, NULL,
			 TPM_CC_Create, TPM_RS_PW, parent_pwd, 0, TPM_RH_NULL, NULL, 0);
	  ...
	  const char *file_name_pub_key, uint32_t *out_handle)
{
	...
	rc = TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out, (COMMAND_PARAMETERS *)&in, NULL,
			 TPM_CC_Load, TPM_RS_PW, parent_pwd, 0, TPM_RH_NULL, NULL, 0);
	       ...
	       const char *sig_key_pwd, uint8_t *qualifying_data, size_t qualifying_data_len)
{
	...
	if (qualifying_data != NULL) {
		if (TPM_RC_SUCCESS != (rc = TSS_TPM2B_Create(&in.qualifyingData.b, qualifying_data,
							     qualifying_data_len, sizeof(TPMT_HA))))
			goto err;
	} else
	...
	do {
		rc = TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out,
				 (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_Quote, TPM_RS_PW,
				 sig_key_pwd, 0, TPM_RH_NULL, NULL, 0);
		  ...
		  TPMI_DH_PERSISTENT persist_handle)
{
	...
	do {
		rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL,
				 TPM_CC_EvictControl, TPM_RS_PW, auth_pwd, 0, TPM_RH_NULL, NULL, 0);
	} while (TPM_RC_RETRY == rc);
		...
		uint8_t *out_buffer, size_t *out_length)
{
	...
	do {
		rc = TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out,
				 (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_RSA_Encrypt, TPM_RH_NULL,
				 NULL, 0);
	} while (TPM_RC_RETRY == rc);
		...
		size_t in_length, uint8_t *out_buffer, size_t *out_length)
{
	...
	do {
		rc = TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out,
				 (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_RSA_Decrypt, TPM_RS_PW,
				 key_pwd, 0, TPM_RH_NULL, NULL, 0);
	} while (TPM_RC_RETRY == rc);
...
tpm2_hierarchychangeauth(TPMI_RH_HIERARCHY hierarchy, const char *old_pwd, const char *new_pwd)
{
	...
	if (new_pwd == NULL)
		in.newAuth.b.size = 0;
		 ...
		 (rc = TSS_TPM2B_StringCopy(&in.newAuth.b, new_pwd, sizeof(TPMU_HA))))
		return rc;
	...
	do {
		rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL,
				 TPM_CC_HierarchyChangeAuth, se_handle, 0,
				 TPMA_SESSION_DECRYPT | TPMA_SESSION_CONTINUESESSION, TPM_RH_NULL,
				 NULL, 0);
	} while (TPM_RC_RETRY == rc);
...
tpm2_getrandom_new(size_t rand_length)
{
	...
	do {
		...
		rc = TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out,
				 (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_GetRandom, se_handle, NULL,
				 TPMA_SESSION_ENCRYPT | TPMA_SESSION_CONTINUESESSION, TPM_RH_NULL,
				 NULL, 0);
		if (rc != TPM_RC_SUCCESS)
			break;
...
tpm2_pcrread_new(TPMI_DH_PCR pcr_index, TPMI_ALG_HASH hash_alg)
{
	...
	do {
		rc = TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out,
				 (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_PCR_Read, TPM_RH_NULL,
				 NULL, 0);
	} while (TPM_RC_RETRY == rc);
		    ...
		    const char *hierarchy_pwd, const char *nv_pwd, uint8_t *policy_digest)
{
	...
	if (policy_digest) {
		...
		memcpy(&in.publicInfo.nvPublic.authPolicy.b.buffer, policy_digest,
		       TPM2D_DIGEST_SIZE);
	      ...
	      size_t data_length)
{
	...
	do {
		rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL,
				 TPM_CC_NV_Write, se_handle, NULL,
				 TPMA_SESSION_DECRYPT | TPMA_SESSION_CONTINUESESSION, TPM_RH_NULL,
				 NULL, 0);
	} while (TPM_RC_RETRY == rc);
