...
tpm2d_marshal_structure_new(void *structure, MarshalFunction_t marshal_function, size_t *size)
{
	...
	if (TPM_RC_SUCCESS != TSS_Structure_Marshal(&bin_stream, &written_size, structure, marshal_function)) {
...
tpm2_startup(TPM_SU startup_type)
{
	...
	rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_Startup, TPM_RH_NULL, NULL, 0);
...
tpm2_selftest(void)
{
	...
	rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_SelfTest, TPM_RH_NULL, NULL, 0);
...
tpm2_clear(const char *lockout_pwd)
{
	...
	rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_Clear, TPM_RS_PW, lockout_pwd, 0,
			 TPM_RH_NULL, NULL, 0);
...
tpm2_dictionaryattacklockreset(const char *lockout_pwd)
{
	...
	rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_DictionaryAttackLockReset,
			 TPM_RS_PW, lockout_pwd, 0, TPM_RH_NULL, NULL, 0);
...
TPM_RC
tpm2_startauthsession(TPM_SE session_type, TPMI_SH_AUTH_SESSION *out_session_handle, TPMI_DH_OBJECT bind_handle,
		      const char *bind_pwd)
{
	TPM_RC rc;
	...
	rc = TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out, (COMMAND_PARAMETERS *)&in,
			 (EXTRA_PARAMETERS *)&extra, TPM_CC_StartAuthSession, TPM_RH_NULL, NULL, 0);
...
tpm2_policyauthvalue(TPMI_SH_POLICY se_handle)
{
	...
	rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_PolicyAuthValue, TPM_RH_NULL, NULL,
			 0);
...
TPM_RC
tpm2_policypcr(TPMI_SH_AUTH_SESSION se_handle, uint32_t pcr_mask, tpm2d_pcr_t *pcrs[], size_t pcrs_size)
{
	TPM_RC rc;
	PolicyPCR_In in;
	...
	rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_PolicyPCR, TPM_RH_NULL, NULL, 0);
...
tpm2_policygetdigest(TPMI_SH_POLICY se_handle, uint8_t *out_digest, size_t out_digest_len)
{
	...
	rc = TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out, (COMMAND_PARAMETERS *)&in, NULL,
			 TPM_CC_PolicyGetDigest, TPM_RH_NULL, NULL, 0);
...
tpm2_policyrestart(TPMI_SH_POLICY se_handle)
{
	...
	rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_PolicyRestart, TPM_RH_NULL, NULL,
			 0);
...
tpm2_flushcontext(TPMI_DH_CONTEXT handle)
{
	...
	rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_FlushContext, TPM_RH_NULL, NULL, 0);
...
tpm2_fill_rsa_details(TPMT_PUBLIC *out_public_area, tpm2d_key_type_t key_type)
{
	...
	case TPM2D_KEY_TYPE_SIGNING_EK:
		...
		out_public_area->parameters.rsaDetail.scheme.details.rsassa.hashAlg = TPM2D_HASH_ALGORITHM;
		break;
	default:
...
tpm2_fill_ecc_details(TPMT_PUBLIC *out_public_area, tpm2d_key_type_t key_type)
{
	...
	case TPM2D_KEY_TYPE_SIGNING_EK:
		...
		out_public_area->parameters.eccDetail.scheme.details.ecdsa.hashAlg = TPM2D_HASH_ALGORITHM;
		out_public_area->parameters.eccDetail.kdf.details.mgf1.hashAlg = TPM2D_HASH_ALGORITHM;
		out_public_area->parameters.eccDetail.curveID = TPM2D_CURVE_ID;
		out_public_area->parameters.eccDetail.kdf.scheme = TPM_ALG_NULL;
		break;
	case TPM2D_KEY_TYPE_STORAGE_U:
...
TPM_RC
tpm2_createprimary_asym(TPMI_RH_HIERARCHY hierachy, tpm2d_key_type_t key_type, const char *hierachy_pwd,
			const char *key_pwd, const char *file_name_pub_key, uint32_t *out_handle)
{
	TPM_RC rc = TPM_RC_SUCCESS;
	...
	else if (TPM_RC_SUCCESS !=
		 (rc = TSS_TPM2B_StringCopy(&in.inSensitive.sensitive.userAuth.b, key_pwd, sizeof(TPMU_HA))))
		return rc;
	in.inSensitive.sensitive.data.t.size = 0;

	// fill in TPM2B_PUBLIC (and overwrite object_attrs)
	if (TPM_RC_SUCCESS != (rc = tpm2_public_area_helper(&in.inPublic.publicArea, object_attrs, key_type)))
		return rc;
	...
	rc = TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out, (COMMAND_PARAMETERS *)&in, NULL,
			 TPM_CC_CreatePrimary, TPM_RS_PW, hierachy_pwd, 0, TPM_RH_NULL, NULL, 0);
	...
	if (file_name_pub_key) {
		rc = TSS_File_WriteStructure(&out.outPublic, (MarshalFunction_t)TSS_TPM2B_PUBLIC_Marshal,
					     file_name_pub_key);
	}
...
#ifndef TPM2D_NVMCRYPT_ONLY
TPM_RC
tpm2_create_asym(TPMI_DH_OBJECT parent_handle, tpm2d_key_type_t key_type, uint32_t object_vals, const char *parent_pwd,
		 const char *key_pwd, const char *file_name_priv_key, const char *file_name_pub_key,
		 const char *file_name_tss_key)
{
	TPM_RC rc = TPM_RC_SUCCESS;
	Create_In in;
	Create_Out out;
	...
	rc = TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_Create,
			 TPM_RS_PW, parent_pwd, 0, TPM_RH_NULL, NULL, 0);
	...
	if (file_name_priv_key) {
		if (TPM_RC_SUCCESS !=
		    (rc = TSS_File_WriteStructure(&out.outPrivate, (MarshalFunction_t)TSS_TPM2B_PRIVATE_Marshal,
						  file_name_priv_key))) {
			return rc;
		}
	}
	...
	if (file_name_tss_key) {
		...
		tpm2d_openssl_write_tpmfile(file_name_tss_key, pubkey, pubkey_len, privkey, privkey_len,
					    key_pwd == NULL, parent_handle, NULL);
	}
	  ...
	  const char *file_name_pub_key, uint32_t *out_handle)
{
	...
	if (TPM_RC_SUCCESS !=
	    (rc = TSS_File_ReadStructure(&in.inPrivate, (UnmarshalFunction_t)TSS_TPM2B_PRIVATE_Unmarshalu,
					 file_name_priv_key)))
		return rc;

	if (TPM_RC_SUCCESS !=
	    (rc = TSS_File_ReadStructureFlag(&in.inPublic, (UnmarshalFunctionFlag_t)TSS_TPM2B_PUBLIC_Unmarshalu, false,
					     file_name_pub_key)))
		return rc;

	rc = TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_Load,
			 TPM_RS_PW, parent_pwd, 0, TPM_RH_NULL, NULL, 0);
...
tpm2_pcrextend(TPMI_DH_PCR pcr_index, TPMI_ALG_HASH hash_alg, const uint8_t *data, size_t data_len)
{
	...
	rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_PCR_Extend, TPM_RS_PW, NULL, 0,
			 TPM_RH_NULL, NULL, 0);
	       ...
	       uint8_t *qualifying_data, size_t qualifying_data_len)
{
	...
	if (qualifying_data != NULL) {
		if (TPM_RC_SUCCESS != (rc = TSS_TPM2B_Create(&in.qualifyingData.b, qualifying_data, qualifying_data_len,
							     sizeof(TPMT_HA))))
			goto err;
	} else
	...
	do {
		rc = TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out, (COMMAND_PARAMETERS *)&in, NULL,
				 TPM_CC_Quote, TPM_RS_PW, sig_key_pwd, 0, TPM_RH_NULL, NULL, 0);
...
tpm2_evictcontrol(TPMI_RH_HIERARCHY auth, char *auth_pwd, TPMI_DH_OBJECT obj_handle, TPMI_DH_PERSISTENT persist_handle)
{
	...
	do {
		rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_EvictControl, TPM_RS_PW,
				 auth_pwd, 0, TPM_RH_NULL, NULL, 0);
	} while (TPM_RC_RETRY == rc);
...
TPM_RC
tpm2_rsaencrypt(TPMI_DH_OBJECT key_handle, uint8_t *in_buffer, size_t in_length, uint8_t *out_buffer,
		size_t *out_length)
{
	TPM_RC rc = TPM_RC_SUCCESS;
	RSA_Encrypt_In in;
	...
	do {
		rc = TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out, (COMMAND_PARAMETERS *)&in, NULL,
				 TPM_CC_RSA_Encrypt, TPM_RH_NULL, NULL, 0);
	} while (TPM_RC_RETRY == rc);
...
TPM_RC
tpm2_rsadecrypt(TPMI_DH_OBJECT key_handle, const char *key_pwd, uint8_t *in_buffer, size_t in_length,
		uint8_t *out_buffer, size_t *out_length)
{
	...
	do {
		rc = TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out, (COMMAND_PARAMETERS *)&in, NULL,
				 TPM_CC_RSA_Decrypt, TPM_RS_PW, key_pwd, 0, TPM_RH_NULL, NULL, 0);
	} while (TPM_RC_RETRY == rc);
...
tpm2_hierarchychangeauth(TPMI_RH_HIERARCHY hierarchy, const char *old_pwd, const char *new_pwd)
{
	...
	if (new_pwd == NULL)
		in.newAuth.b.size = 0;
	else if (TPM_RC_SUCCESS != (rc = TSS_TPM2B_StringCopy(&in.newAuth.b, new_pwd, sizeof(TPMU_HA))))
		return rc;
	...
	do {
		rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_HierarchyChangeAuth,
				 se_handle, 0, TPMA_SESSION_DECRYPT | TPMA_SESSION_CONTINUESESSION, TPM_RH_NULL, NULL,
				 0);
	} while (TPM_RC_RETRY == rc);
...
tpm2_getrandom_new(size_t rand_length)
{
	...
	do {
		...
		rc = TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out, (COMMAND_PARAMETERS *)&in, NULL,
				 TPM_CC_GetRandom, se_handle, NULL, TPMA_SESSION_ENCRYPT | TPMA_SESSION_CONTINUESESSION,
				 TPM_RH_NULL, NULL, 0);
		if (rc != TPM_RC_SUCCESS)
			break;
...
tpm2_nv_get_data_size(TPMI_RH_NV_INDEX nv_index_handle)
{
	...
	if (TPM_RC_SUCCESS != TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out, (COMMAND_PARAMETERS *)&in, NULL,
...
tpm2_nv_get_max_buffer_size(TSS_CONTEXT *tss_context)
{
	...
	if (TPM_RC_SUCCESS != TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out, (COMMAND_PARAMETERS *)&in, NULL,
					  TPM_CC_GetCapability, TPM_RH_NULL, NULL, 0)) {
		    ...
		    const char *hierarchy_pwd, const char *nv_pwd, uint8_t *policy_digest)
{
	...
	if (nv_pwd == NULL)
		in.auth.b.size = 0;
	else if (TPM_RC_SUCCESS != (rc = TSS_TPM2B_StringCopy(&in.auth.b, nv_pwd, sizeof(TPMU_HA))))
		return rc;
	...
	do {
		rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_NV_DefineSpace,
...
TPM_RC
tpm2_nv_undefinespace(TPMI_RH_HIERARCHY hierarchy, TPMI_RH_NV_INDEX nv_index_handle, const char *hierarchy_pwd)
{
	TPM_RC rc, rc_flush = TPM_RC_SUCCESS;
	TPMI_SH_AUTH_SESSION se_handle;
	...
	do {
		rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_NV_UndefineSpace,
				 //TPM_RS_PW, hierarchy_pwd, 0,
				 se_handle, 0, TPMA_SESSION_CONTINUESESSION, TPM_RH_NULL, NULL, 0);
	} while (TPM_RC_RETRY == rc);
...
TPM_RC
tpm2_nv_write(TPMI_RH_NV_INDEX nv_index_handle, const char *nv_pwd, uint8_t *data, size_t data_length)
{
	TPM_RC rc, rc_flush = TPM_RC_SUCCESS;
	TPMI_SH_AUTH_SESSION se_handle;
	NV_Write_In in;
	...
	do {
		rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_NV_Write, se_handle, NULL,
				 TPMA_SESSION_DECRYPT | TPMA_SESSION_CONTINUESESSION, TPM_RH_NULL, NULL, 0);
	} while (TPM_RC_RETRY == rc);
...
TPM_RC
tpm2_nv_read(TPMI_SH_POLICY se_handle, TPMI_RH_NV_INDEX nv_index_handle, const char *nv_pwd, uint8_t *out_buffer,
	     size_t *out_length)
{
	TPM_RC rc, rc_flush = TPM_RC_SUCCESS;
	...
	do {
		...
		do {
			rc = TSS_Execute(tss_context, (RESPONSE_PARAMETERS *)&out, (COMMAND_PARAMETERS *)&in, NULL,
					 TPM_CC_NV_Read, auth_se_handle, NULL,
					 TPMA_SESSION_ENCRYPT | TPMA_SESSION_CONTINUESESSION, TPM_RH_NULL, NULL, 0);
		} while (TPM_RC_RETRY == rc);
...
tpm2_nv_readlock(TPMI_RH_NV_INDEX nv_index_handle, const char *nv_pwd)
{
	...
	do {
		rc = TSS_Execute(tss_context, NULL, (COMMAND_PARAMETERS *)&in, NULL, TPM_CC_NV_ReadLock,
				 //TPM_RS_PW, nv_pwd, 0,
				 se_handle, 0, TPMA_SESSION_CONTINUESESSION, TPM_RH_NULL, NULL, 0);
	} while (TPM_RC_RETRY == rc);
