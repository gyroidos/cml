...
static TPM2_RC
nvmcrypt_start_policy_session(TPM2_SE session_type, TPMI_SH_AUTH_SESSION *session_handle,
...
nvmcrypt_create_policy(void)
{
	...
	ret = nvmcrypt_start_policy_session(TPM2_SE_TRIAL, &se_trial, ESYS_TR_RH_NULL, NULL);
	IF_FALSE_RETVAL(TPM2_RC_SUCCESS == ret, ret);
...
nvmcrypt_load_key_new(const char *fde_key_pw)
{
	...
	if (tpm2_nv_get_data_size(TPM2D_FDE_NV_HANDLE) != 0) {
		if (secure_boot) {
			...
			ret = nvmcrypt_start_policy_session(TPM2_SE_POLICY, &se_handle, ESYS_TR_RH_NULL,
							    NULL);
		} else {
			// let nv_read do its own auth session
			se_handle = ESYS_TR_RH_NULL;
			ret = TPM2_RC_SUCCESS;
		}
		if (TPM2_RC_SUCCESS == ret) {
			...
			if ((ret & TPM2_RC_AUTH_FAIL) == TPM2_RC_AUTH_FAIL) {
				fde_state = FDE_AUTH_FAILED;
			} else if ((ret & TPM2_RC_POLICY_FAIL) == TPM2_RC_POLICY_FAIL) {
				fde_state = FDE_AUTH_FAILED;
			} else if ((ret & TPM2_RC_NV_LOCKED) == TPM2_RC_NV_LOCKED) {
	...
	if (secure_boot) {
		if (TPM2_RC_SUCCESS != (ret = nvmcrypt_start_policy_session(
					       TPM2_SE_POLICY, &se_handle, ESYS_TR_RH_NULL, NULL))) {
	...
	if (TPM2_RC_SUCCESS != (ret = tpm2_nv_read(se_handle, TPM2D_FDE_NV_HANDLE, fde_key_pw,
...
{
	if (TPM2_RC_SUCCESS == tpm2_nv_readlock(TPM2D_FDE_NV_HANDLE, fde_pw))
		fde_state = FDE_KEY_ACCESS_LOCKED;
	return fde_state;
}
...
nvmcrypt_init(bool use_secure_boot_policy)
{
	...
	if (use_secure_boot_policy) {
		if (TPM2_RC_SUCCESS != nvmcrypt_create_policy())
