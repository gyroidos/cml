...
c_service_send_container_cfg_name_proto(c_service_t *service)
{
	...
	INFO("Sending container config name %s to container %s", container_get_name(service->container),
...
c_service_send_container_cfg_dns_proto(c_service_t *service)
{
	...
	INFO("Sending container config dns %s to container %s", container_get_dns_server(service->container),
	     container_get_description(service->container));
...
c_service_handle_received_message(c_service_t *service, const ServiceToCmldMessage *message)
{
	...
	case SERVICE_TO_CMLD_MESSAGE__CODE__CONNECTIVITY_CHANGE:
		INFO("Received connectivity change message `%d' from container %s", message->connectivity,
		     container_get_description(service->container));
	...
	case SERVICE_TO_CMLD_MESSAGE__CODE__AIRPLANE_MODE_CHANGED:
		INFO("Received airplane mode change message `%d' from container %s", message->airplane_mode,
		     container_get_description(service->container));
	...
	case SERVICE_TO_CMLD_MESSAGE__CODE__IMEI_MAC_PHONENO:
		INFO("Received imei: %s, mac: %s and phoneno.: %s from container %s",
		     ...
		     message->phonenumber ? message->phonenumber : "", container_get_description(service->container));
		if (message->imei)
...
c_service_cb_accept(int fd, unsigned events, event_io_t *io, void *data)
{
	...
	service->event_io_sock_connected =
		event_io_new(service->sock_connected, EVENT_IO_READ, &c_service_cb_receive_message, service);
...
c_service_start_pre_exec(c_service_t *service)
{
	...
	service->connectivity_observer =
		container_register_observer(service->container, &c_service_connectivity_observer_cb, service);
	if (!service->connectivity_observer) {
	...
	service->airplane_mode_observer =
		container_register_observer(service->container, &c_service_airplane_mode_observer_cb, service);
	if (!service->airplane_mode_observer) {
