...
int c_cgroups_devices_allow_audio(c_cgroups_t *cgroups)
{
	...
	if (c_cgroups_devices_allow_list(cgroups, hardware_get_devices_whitelist_audio()) < 0) {
		ERROR("Could not allow audio devices for container %s", container_get_description(cgroups->container));
		return -1;
	}
	return 0;
}
...
int c_cgroups_devices_deny_audio(c_cgroups_t *cgroups)
{
	...
	if (c_cgroups_devices_deny_list(cgroups, hardware_get_devices_whitelist_audio()) < 0) {
		ERROR("Could not deny audio devices for container %s", container_get_description(cgroups->container));
		return -1;
	}
				       ...
				       void *data)
{
	...
	if (!strncmp(state, "THAWED", strlen("THAWED")) &&
	...
	} else if (!strncmp(state, "FREEZING", strlen("FREEZING")) && container_state != CONTAINER_STATE_FREEZING) {
		...
		cgroups->freeze_timer =
			event_timer_new(CGROUPS_FREEZER_RETRY_INTERVAL, -1, &c_cgroups_freeze_timeout_cb, cgroups);
		event_add_timer(cgroups->freeze_timer);

		container_set_state(cgroups->container, CONTAINER_STATE_FREEZING);
	} else if (!strncmp(state, "FROZEN", strlen("FROZEN")) && container_state != CONTAINER_STATE_FROZEN) {
...
int c_cgroups_set_ram_limit(c_cgroups_t *cgroups)
{
	...
	INFO("Trying to set RAM limit of container %s to %d MBytes", container_get_description(cgroups->container),
	...
	INFO("Successfully set RAM limit of container %s to %d MBytes", container_get_description(cgroups->container),
	     container_get_ram_limit(cgroups->container));
					       ...
					       void *data)
{
	...
	if (mask & IN_CREATE) {
		...
		if (file_is_dir(path)) {
			...
			event_inotify_t *inotify_dev = event_inotify_new(path, IN_OPEN | IN_CREATE,
	 ...
	 */
	DEBUG("dev in container %s: %s %u %u %s", container_get_description(cgroups->container), type, maj, min,
	      basename(path));
...
static int c_cgroups_devices_dev_dir_foreach_cb(const char *path, const char *name, void *data)
{
	...
	event_inotify_t *inotify_dev =
		event_inotify_new(full_path, IN_OPEN | IN_CREATE, &c_cgroups_devices_watch_dev_dir_cb, cgroups);
	event_add_inotify(inotify_dev);
...
UNUSED static void c_cgroups_devices_watch_dev_dir(c_cgroups_t *cgroups)
{
	...
	event_inotify_t *inotify_dev =
		event_inotify_new(dev_path, IN_OPEN | IN_CREATE, &c_cgroups_devices_watch_dev_dir_cb, cgroups);
	event_add_inotify(inotify_dev);
...
int c_cgroups_start_pre_clone(c_cgroups_t *cgroups)
{
	...
	if (mount("cgroup", CGROUPS_FOLDER, "tmpfs", MS_NOEXEC | MS_NODEV | MS_NOSUID | MS_RELATIME, "mode=755") ==
		    -1 &&
	    errno != EBUSY) {
...
int c_cgroups_start_post_clone(c_cgroups_t *cgroups)
{
	...
	char *freezer_state_path = mem_printf("%s/freezer/%s/freezer.state", CGROUPS_FOLDER,
					      uuid_string(container_get_uuid(cgroups->container)));
	cgroups->inotify_freezer_state =
		event_inotify_new(freezer_state_path, IN_MODIFY, &c_cgroups_freezer_state_cb, cgroups);
