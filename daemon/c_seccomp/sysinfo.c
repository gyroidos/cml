			  ...
			  struct seccomp_notif_resp *resp)
{
	...
	if (-1 == (ret_sysinfo = namespace_exec(
			   compartment_get_pid(seccomp->compartment),
			   CLONE_NEWALL & (~CLONE_NEWPID) & (~CLONE_NEWCGROUP) & (~CLONE_NEWNS), 0,
			   0, c_seccomp_do_sysinfo_fork, &sysinfo_params))) {
		ERROR_ERRNO("Failed to execute sysinfo");
		goto out;
	}
