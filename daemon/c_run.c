		...
		if (pid == exec_pid) {
			...
			if (WIFEXITED(status)) {
				INFO("Exec'ed process in container %s terminated (status=%d)",
				     container_get_description(run->container), WEXITSTATUS(status));
			...
			DEBUG("Reaped a descendant process with PID %d of injected process in container %s", pid,
...
do_exec(c_run_t *run)
{
	...
	TRACE("[EXEC]: Executing command %s in process with PID: %d, PGID: %d, PPID: %d", run->cmd, getpid(),
