...
guestos_mgr_load_operatingsystems_cb(const char *path, const char *name, UNUSED void *data)
{
	...
	smartcard_crypto_verify_result_t verify_result =
		smartcard_crypto_verify_file_block(cfg_file, sig_file, cert_file, GUESTOS_MGR_VERIFY_HASH_ALGO);
	...
	default:
		...
		ERROR("Signature verification failed (%d) while loading GuestOS config %s, skipping.", verify_result,
		      cfg_file);
		res = 1;
		goto cleanup_files;
	}
		      ...
		      const char *cert_file, UNUSED smartcard_crypto_hashalgo_t hash_algo, UNUSED void *data)
{
	...
	case VERIFY_LOCALLY_SIGNED:
		...
		// fallthrough
	default:
		ERROR("Signature verification failed (%d) for pushed GuestOS config %s, skipping.", verify_result,
		      cfg_file);
	...
	if (old_os) {
		...
		if (os_ver <= old_ver) {
			WARN("Skipping update of GuestOS %s version %" PRIu64 " to older/same version %" PRIu64 ".",
			     os_name, old_ver, os_ver);
			goto cleanup_os;
		}
		DEBUG("Updating GuestOS config for %s from v%" PRIu64 " to v%" PRIu64 ".", os_name, old_ver, os_ver);
	} else {
		// Fresh install
	...
	if (file_move(cfg_file, guestos_get_cfg_file(os), GUESTOS_MGR_FILE_MOVE_BLOCKSIZE) < 0) {
		ERROR_ERRNO("Failed to move GuestOS config %s to %s", cfg_file, guestos_get_cfg_file(os));
		goto cleanup_purge;
	}
	if (file_move(sig_file, guestos_get_sig_file(os), GUESTOS_MGR_FILE_MOVE_BLOCKSIZE) < 0) {
		ERROR_ERRNO("Failed to move GuestOS config signature %s to %s", sig_file, guestos_get_sig_file(os));
		goto cleanup_purge;
	}
	if (file_move(cert_file, guestos_get_cert_file(os), GUESTOS_MGR_FILE_MOVE_BLOCKSIZE) < 0) {
		ERROR_ERRNO("Failed to move GuestOS config certificate %s to %s", cert_file, guestos_get_cert_file(os));
		goto cleanup_purge;
	}
...
guestos_mgr_register_localca(unsigned char *cacert, size_t cacertlen)
{
	...
	if ((ret = file_move(tmp_cacert_file, LOCALCA_ROOT_CERT, GUESTOS_MGR_FILE_MOVE_BLOCKSIZE)) < 0) {
		ERROR_ERRNO("Failed to move localca root certificate %s to %s", tmp_cacert_file, LOCALCA_ROOT_CERT);
	} else {
		INFO("Successfully installed localca root certificate %s to %s", tmp_cacert_file, LOCALCA_ROOT_CERT);
	}
...
guestos_mgr_register_newca(unsigned char *cacert, size_t cacertlen)
{
	...
	if ((ret = file_move(tmp_cacert_file, cacert_file, GUESTOS_MGR_FILE_MOVE_BLOCKSIZE)) < 0) {
		ERROR_ERRNO("Failed to move new ca certificate %s to %s", tmp_cacert_file, cacert_file);
	} else {
		INFO("Successfully installed new ca certificate %s to %s", tmp_cacert_file, cacert_file);
	}
out:
